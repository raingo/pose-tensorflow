...
std::vector<int> nms_grid_cpp(const float *unProbValues, size_t height,
                              size_t width, unsigned char *gridData,
  ...
  for (int i = 0; i < num_locs; ++i) indices[i] = i;

  std::sort(indices.begin(), indices.end(), [&unProbValues](int a, int b) {
    return unProbValues[a] > unProbValues[b];
  });

  // std::cout << unProbValues[indices[0]] << " " << unProbValues[indices[1]] <<
  // " " << unProbValues[indices[2]] << std::endl;
  ...
  for (int k = 0; k < num_locs; ++k) {
    int idx = indices[k];
    if (wasted[idx] == 1) continue;

    if (unProbValues[idx] < nms_prob_thresh) break;
    ...
    // std::cout << " jj " << jj << " start_j " << start_j << "  end_j " <<
    // end_j << std::endl;
...
std::vector<int> nms_grid_cpp_test(float *unProbValues, size_t height,
                                   size_t width) {
  std::array<size_t, 2> shape;
  shape[0] = height;
  shape[1] = width;
